...
export default function InspectionForm({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch(`/api/vehicle/${token}`).then(r => r.json()).then(d => setVehicle(d.vehicle ?? null));
  }, [token]);
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (employeeMode) form.set("employee_name", selectedEmployee);
  ...
  if (!vehicle) return <section className="card"><h1>Vehicle inspection</h1><p>Loading vehicle…</p></section>;
  ...
  return <section className="card">
    ...
    <form onSubmit={submit}>
          ...
          <div className="employee-grid">
            {employeeNames.map((name) => (
              <button
                type="button"
                key={name}
                className={`employee-button${selectedEmployee === name ? " selected" : ""}`}
      ...
      <label className="field"><span>Odometer / hours</span><input name="odometer" required inputMode="decimal" /></label>
        ...
        {photoFields.map(([name, label]) => {
          ...
          return (
            <div className={`camera-field${preview ? " complete" : ""}`} key={name}>
              ...
              {preview && (
                <img className="camera-preview" src={preview} alt={`${label} preview`} />
              )}
              <label className={`camera-button${preview ? " retake" : ""}`} htmlFor={`photo-${name}`}>
              ...
              <input
                ...
                accept="image/*"
                capture="environment"
                onChange={(event) => handlePhotoChange(name, event)}
              />
              {preview ? (
                <small className="photo-complete">✓ Photo complete</small>
              ) : (
                <small className="small">Optional for now. Use the rear camera and show the entire area clearly.</small>
              )}
            </div>
          );
        })}
      ...
      {status && <p className={status === "Inspection submitted successfully." ? "notice success" : status === "Submitting inspection…" ? "notice" : "notice error"}>{status}</p>}
    </form>
  </section>;
}
